import { Driver, DrowsinessEvent, DriverWithStatus } from '@/types/driver';
...
const generateEvents = (): DrowsinessEvent[] => {
...
export const mockEvents: DrowsinessEvent[] = generateEvents();

export const getDriversWithStatus = (): DriverWithStatus[] => {
  return mockDrivers.map(driver => {
    const driverEvents = mockEvents.filter(e => e.driverId === driver.driverId);
    const totalDrowsinessTime = driverEvents.reduce((acc, e) => acc + (e.duration || 0), 0);
    const lastEvent = driverEvents[0];
    
    // Simulate current status
    const statuses: ('alert' | 'drowsy' | 'offline')[] = ['alert', 'alert', 'alert', 'drowsy', 'offline'];
    const randomStatus = statuses[Math.floor(Math.random() * statuses.length)];
    
    return {
      ...driver,
      currentStatus: driver.driverId === 'DRV001' ? 'drowsy' : randomStatus,
